...
public class TftpEncoderDecoder implements MessageEncoderDecoder<byte[]> {
    ...
    public byte[] decodeNextByte(byte nextByte) {
        ...
        if(packetType==null) //opCode is not determined yet.
        {
            if(opcode==-1)
            {
        ...
        else //opCode is determined, handling each packet differently.
        {
            bytes.add(nextByte);
            switch(packetType)
            {
        ...
        if(finishedMsg)
        { //get here when completed full message. constructs the new message int byte[] and resets the state to
            //be able to accept new message.
            packetPart = 0;
    ...
    @Override
    public byte[] encode(byte[] message) { //pretty much does nothing.
        return message;
    }
}